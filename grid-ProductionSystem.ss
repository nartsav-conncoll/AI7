...
(define rules 
  '(
    ...
    (r2 (if (current x) (path y) (adjacent y) (not visited y) (not obstacle y))
        (delete (current x) )
        (add (visited x) (path x) (delete_adjacents) (backtrack_to y)))
    ...
    (r7 (if (current x) (adjacent y) (not path y) (not visited y) (obstacle y) (height y low) (stable y))
        (delete (current x))
        (add (path x) (delete_adjacents) (move_to y)))
    
    (r8 (if (current x) (path y) (adjacent y) (not visited y) (obstacle y) (height y low) (stable y))
        (delete (current x) )
        (add (visited x) (path x) (delete_adjacents) (backtrack_to y)))
))
